...
public class FarmScreen extends JPanel implements ActionListener {
	...
	private boolean checkFarmGameIsWin = false;
	...
	JLabel farmGameAlertLabel = new JLabel();
	JButton farmGameAlertOKBtn = new JButton();
	ImageIcon farmGameAlertPic = resizeImage(615, 450, new ImageIcon("../picture/farmGameAlert.PNG"));
	ImageIcon farmGameAlertOKBtnPic = resizeImage(118, 37, new ImageIcon("../picture/farmGameAlertOKBtn.PNG"));
	
	public void getFarmGameIsWin(boolean check)
	{
		if(check == true)
		{
			System.out.println("除蟲遊戲成功");
			Date mainDate = new Date();
			farm.setLastKillBugDate(mainDate);
			checkFarmGameIsWin = check;
		}
		else {
			System.out.println("除蟲遊戲失敗");
			checkFarmGameIsWin = check;
		}
	...
	public void reload()
	{
		...
		farmGameAlertOKBtn.setVisible(false);
		farmGameAlertLabel.setVisible(false);
	...
	public void LandButton(Farm farm, WareHouse wareHouse)
	{
		...
		if(farm.checkLand()) 
		{
			...
			{
				//System.out.println("???");
				//----------------跳出提醒除蟲視窗(每個土地都有一個提醒視窗)---------------------------
				System.out.println("除蟲按鈕亮燈了!!");
				farmGameAlertOKBtn.setEnabled(true);
				farmGameAlertOKBtn.setVisible(true);
				farmGameAlertLabel.setVisible(true);
				//-----------------------------------------------
	...
	public void setfarmObject(String [][] farmStore) throws ParseException //讀取Farm中的所有資料 (讀檔用)
	{
		...
		if(count >= 1) farm.setLastKillBugDate(sdf.parse(farmStore[12][1]));
	}
	public FarmScreen(Main mainFrame, WareHouse wareHouse) {
		...
		for(int i = 0; i < 12; i++)
			clickCheck[i] = false;
		//-----------------提醒除蟲視窗-----------------------
		farmGameAlertOKBtn.setBounds(350, 225, 400, 300);
		farmGameAlertLabel.setBounds(280, 91, 600, 500);
		
		this.add(farmGameAlertOKBtn);
		farmGameAlertOKBtn.setIcon(farmGameAlertOKBtnPic);	
		farmGameAlertOKBtn.setVisible(false);
		farmGameAlertOKBtn.setOpaque(false);
		farmGameAlertOKBtn.setContentAreaFilled(false);
		farmGameAlertOKBtn.setFocusPainted(false);
		farmGameAlertOKBtn.setBorder(null);
		
		farmGameAlertOKBtn.addMouseListener(new MouseAdapter() {
			@Override
            ...
            public void mouseEntered(MouseEvent arg0) {
				farmGameAlertOKBtn.setIcon(resizeImage(farmGameAlertOKBtn.getIcon().getIconWidth()+10,farmGameAlertOKBtn.getIcon().getIconHeight()+10,(ImageIcon)farmGameAlertOKBtn.getIcon()));
				buttonSound();
			} 
            @Override
            public void mouseExited(MouseEvent arg0) {
            	farmGameAlertOKBtn.setIcon(resizeImage (118, 37,new ImageIcon("../picture/farmGameAlertOKBtn.png")));
            }   
		});
		farmGameAlertLabel.setIcon(farmGameAlertPic);	
		this.add(farmGameAlertLabel);
		farmGameAlertLabel.setVisible(false);
		//------------------------------------------------------------
		//收割button
