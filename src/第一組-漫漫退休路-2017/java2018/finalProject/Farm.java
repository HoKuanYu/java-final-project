...
public class Farm
{
	...
	public Boolean harvest(int index)
	{
		...
		int randomCrop = 0;
		// 把收割的作物加到倉庫
		if(farmLand.get(storeCropArray[index]).getName() == "玉米")
			randomCrop = random.nextInt(4)+1;
		else if(farmLand.get(storeCropArray[index]).getName() == "小麥")
			randomCrop = random.nextInt(6)+1;
		else randomCrop = random.nextInt(3)+1;
		for(int i = 0; i <= randomCrop; i++)
	...
	public boolean decideCallRandomCheck()
	{
		...
		if(diff >= 300)  //超過五分鐘直接強迫除蟲
			return true;
		else if(diff >= 30)   //30秒判斷是否除蟲
		{
			if(randomCheck())
				return true;
			else
				setLastKillBugDate(mainDate);  //random失敗才需重新設定除蟲時間
		}
		return false;
	...
	public boolean randomCheck()
	{
		...
		killBugButtonClick = true; //除蟲按鈕有亮
		return true;  
	}
