...
{  
    ...
    public FarmGameScreen(Main mainFrame, WareHouse warehouse, FarmScreen farmScreen)
    {  
    	...
    	this.farmScreen = farmScreen; // 為了回傳除蟲遊戲是否成功
        this.setSize(1200, 675);    
        this.setLayout(null);//設置框架佈局模式為空（只有這樣，才能知道圖片的真正位置  
      
        // Math.random 會產生 0 ~ 接近 1 的數字 
        restBugNumber = (int)(17+Math.random() * 6);  // 25 ~ 30
        restGrassNumber = (int)(17+Math.random() * 6);
    ...
    public void farmScreenReload()
    {
    	this.farmScreen.reload();
    }
    //----------------------------------去除按鈕背景-----------------------------
	...
	private void timeMessageAppear()
    {
		...
		//System.out.printf("timeMessage: bug=%d, grass=%d", restBugNumber, restGrassNumber);
	    if(restBugNumber == 0 && restGrassNumber == 0)
	    {
		    ...
		    this.farmScreen.getFarmGameIsWin(true);
	    ...
	    else
	    {
		    ...
		    this.farmScreen.getFarmGameIsWin(false);
		    backToFarmBtn.setVisible(true);
		    farmScreen.reload();
	    }
    }
    ...
    {  
        ...
        if(i == MouseEvent.BUTTON1){
        	...
        	//System.out.printf("bug=%d, grass=%d", restBugNumber, restGrassNumber);
